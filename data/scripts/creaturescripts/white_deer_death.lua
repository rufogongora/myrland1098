...
function creatureevent.onDeath(creature, corpse, lasthitkiller, mostdamagekiller, lasthitunjustified, mostdamageunjustified)
	...
	if not isMonster(creature) or isSummon(creature) then
		return true
	end
	...
	for i = 1, #config do
		if chance <= config[i].chance then
			local spawnMonster = Game.createMonster(config[i].monster,getCreaturePos(creature), true, true)
			if spawnMonster then
				spawnMonster:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
				doCreatureSay(config[i].message, TALKTYPE_MONSTER_SAY)
			end
			break
		end
	end
