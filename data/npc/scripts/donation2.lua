...
------------------------------------------------ confirm yes ------------------------------------------------ 
		...
		jugador = ("SELECT `account_id` FROM `players` WHERE `id` = " .. getPlayerGUID(cid) .. ";")
		local rows = db.getResult("SELECT `account_id` FROM `vip_accounts` WHERE id = " .. db.escapeString(jugador) .. ";")  
            talk_state = 0   
            if getPlayerItemCount(cid,5022) >= 30 then 
                if(rows:getID() ~= 0) then
                    if doPlayerTakeItem(cid,5022,30) == 0 then 
                        ...
                        db.executeQuery('UPDATE `accounts` SET `vip` = 1 WHERE `id` = ("SELECT `account_id` FROM `players` WHERE `id` = " .. db.escapeString(jugador) .. ") ;')
                    end 
                else 
